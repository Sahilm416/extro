...
<Callout type="warn" title="There is no safe place for a secret">
...
Precedence, lowest to highest: `.env` < `.env.[mode]` < `.env.local` < `.env.[mode].local`. `extro dev` loads the `development` set, `extro build` loads `production`. Real process env (CI) always wins over `.env` files. Commit a `.env.example` as the template; everything else is git-ignored by default.
...
<Cards>
  <Card title="Manifest Generation" href="/docs/guide/manifest" description="Where the CRX key lands and how fields resolve." />
</Cards>
