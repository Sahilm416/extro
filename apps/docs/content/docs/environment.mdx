...
`import.meta.env` is typed automatically. Any project that imports `@extrojs/react` (every routable surface does) picks up the ambient types, so `import.meta.env.EXTRO_PUBLIC_*` resolves as `string | undefined` with no setup.
