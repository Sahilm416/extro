...
`import.meta.env` is typed automatically. Any project that imports `@extrojs/router` (every routable surface does, and it re-exports the ambient typing from `@extrojs/react`) picks up the types, so `import.meta.env.EXTRO_PUBLIC_*` resolves as `string | undefined` with no setup.
...
To name specific vars instead of the generic `EXTRO_PUBLIC_*` index, augment `ImportMetaEnv` yourself.
